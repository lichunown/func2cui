...
class ParseFunction(ParseBase):
    def __init__(self, function, opt_map:dict={}, param_map:dict={}, debug=True, help=True):
        super().__init__(help)
    ...
    def __call__(self, args):
        if self.help and '--help' in args or ('-h' in args and '-h' not in self.opt_map):
...
class ParseModule(ParseBase):
    def __init__(self, debug=True, help=True):
        super().__init__(help)
        self.debug = debug
        self.cmd_map = {}
        
    def add_sub_parse(self, cmd, cmd_parse):
        if not isinstance(cmd_parse, ParseBase):
            raise ValueError
        self.cmd_map[cmd] = cmd_parse

    def sub_module(self, cmd, opt_map={}, param_map:dict={}, debug=True, help=True, help_msg=None):
        def decorator(func):
            self.cmd_map[cmd] = ParseFunction(func, opt_map, param_map, debug, help)
            if help:
                if help_msg == '__doc__':
                    self.cmd_map[cmd].help_msg = func.__doc__
                elif isinstance(help_msg, str):
                    self.cmd_map[cmd].help_msg = help_msg
                elif help_msg is None:
                    pass
                else:
                    raise ValueError
            return func
    ...
    def __call__(self, cmds):
        if len(cmds) < 1:
            return self.help_msg
        if self.help and '--help' == cmds[0] or ('-h' == cmds[0] and '-h' not in self.cmd_map):
            return self.help_msg
